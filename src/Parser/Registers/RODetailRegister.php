...
class RODetailRegister extends BaseRegister
{
    ...
    public function __construct()
    {
        ...
        $this->addField(new RegisterField('data_apresentacao', 'N', 26, 31, 6, true, 'AAMMDD – Data em que o RO foi transmitido para a Cielo.', '0'));
        $this->addField(new RegisterField('data_prevista_pagamento', 'N', 32, 37, 6, true, 'AAMMDD – Data prevista de pagamento.', '0'));
        $this->addField(new RegisterField('data_envio_banco', 'N', 38, 43, 6, true, 'AAMMDD – Data em que o arquivo de pagamento foi enviado ao banco.', '0'));
        $this->addField(new RegisterField('sinal_valor_bruto', 'A', 44, 44, 1, true, '+ = credito, - = debito', '0'));
        $this->addField(new RegisterField('valor_bruto', 'N', 45, 57, 13, true, 'Somatória dos valores de venda.', '0'));
        $this->addField(new RegisterField('sinal_comissao', 'N', 58, 58, 1, true, '+ = credito, - = debito.', '0'));
        $this->addField(new RegisterField('valor_comissao', 'N', 59, 71, 13, true, 'Valor da comissão descontada sobre as vendas.', '0'));
        $this->addField(new RegisterField('sinal_valor_rejeitado', 'N', 72, 72, 1, true, '+ = credito, - = debito.', '0'));
        $this->addField(new RegisterField('valor_rejeitado', 'N', 73, 85, 13, true, 'Se houver rejeição, será preenchido com a somatória das transações rejeitadas.', '0'));
        $this->addField(new RegisterField('sinal_valor_liquido', 'N', 86, 86, 1, true, '+ = credito, - = debito.', '0'));
        $this->addField(new RegisterField('valor_liquido', 'N', 87, 99, 13, true, 'Valor das vendas descontado o valor da comissão.', '0'));
        ...
        $this->addField(new RegisterField('data_captura_transacao', 'N', 140, 145, 6, true, 'AAMMDD - Data em que a transação foi capturada da pela Cielo.', '0'));     
        $this->addField(new RegisterField('origem_ajuste', 'A', 146, 147, 2, true, '02 = Ajuste Credito, 03 = Ajuste debito, 04 = Ajuste aluguel.', '0'));   
        $this->addField(new RegisterField('valor_complementar', 'N', 148, 160, 13, true, 'Valor do saque quando o produto for igual a “36” ou valor do Agro Electron para transações dos produtos “22”, “23” ou “25” apresentados na Tabela IV.', '0'));   
        ...
        $this->addField(new RegisterField('valor_bruto_antecipado', 'N', 172, 184, 13, true, 'Valor bruto antecipado, fornecido quando o RO for antecipado/cedido.', '0'));   
        ...
        $this->addField(new RegisterField('taxa_comissao', 'N', 210, 213, 4, true, 'Percentual de comissão aplicado no valor da transação.', '0'));   
        $this->addField(new RegisterField('tarifa', 'N', 214, 218, 5, true, 'Tarifa cobrada por transação.', '0'));   
